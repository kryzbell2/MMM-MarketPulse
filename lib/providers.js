...
function parseAaaFuel(html, region, grade) {
    ...
    const dieselIndex = headers.findIndex((cell) => cell.toLowerCase() === grade.toLowerCase());
    ...
    if (!isFiniteNumber(price) || price <= 0 || price > 20) throw new Error(`AAA current ${grade.toLowerCase()} value was missing or implausible`);
...
function parseAaaDiesel(html, region = "US") {
    return parseAaaFuel(html, region, "Diesel");
}

function parseAaaGas(html) {
    return parseAaaFuel(html, "US", "Regular");
}

async function fetchAaaGas(options = {}) {
    return parseAaaGas(await fetchText("https://gasprices.aaa.com/", options));
}
...
module.exports = { DEFAULT_USER_AGENT, fetchAaaGas, parseAaaGas, fetchAaaDiesel, fetchText, fetchYahooQuote, normalizeDieselRegion, parseAaaDiesel, parseYahooChart };
